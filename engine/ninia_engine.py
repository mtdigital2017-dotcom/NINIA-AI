...
class NiniaEngine:
    ...
    RISK_KEYWORDS = {
        "desinformación": ["desinformación", "misinformation", "disinformation", "fake news"],
        "privacidad infantil": ["privacidad", "privacy", "datos personales", "personal data"],
        "publicidad dirigida": ["publicidad", "advertising", "targeted ads", "marketing"],
        "plataformas digitales": ["plataforma", "platform", "redes sociales", "social media"],
        "ia generativa": ["ia generativa", "generative ai", "deepfake", "chatbot"],
        "ciberacoso": ["ciberacoso", "cyberbullying", "bullying"],
    }
    ...
    def infer_source_entity(self, file_name: str, text: str) -> str:
        candidates = ["UNESCO", "UNICEF", "ITU", "OECD", "OCDE", "ONU", "CRC", "EU", "DSA", "AI ACT"]
    ...
    def build_knowledge_object(self, file_path: Path | str, text: str, metadata: dict | None = None) -> dict:
        metadata = metadata or {}
        path = Path(file_path)
        return {
            "id": str(uuid.uuid4()),
            "title": metadata.get("title") or path.stem.replace("_", " ").replace("-", " "),
            "source_entity": metadata.get("source_entity") or self.infer_source_entity(path.name, text),
            "year": metadata.get("year") or self.infer_year(path.name, text),
            "document_type": metadata.get("document_type") or path.suffix.lower().replace(".", "").upper(),
            "source_url_or_doi": metadata.get("source_url_or_doi", ""),
            "main_topic": metadata.get("main_topic") or "Protección integral de niñas, niños y adolescentes en entornos digitales",
            "relation_to_ninia": metadata.get("relation_to_ninia") or "Documento procesado por NINIA para análisis y construcción de conocimiento.",
            "digital_risks": self.detect_digital_risks(text),
            "target_population": metadata.get("target_population") or "Niñas, niños y adolescentes",
            "evidence_level": metadata.get("evidence_level") or "NO_CLASIFICADO",
    ...
    def save_knowledge_object(self, knowledge_object: dict) -> Path:
        ...
        output.write_text(json.dumps(knowledge_object, ensure_ascii=False, indent=2), encoding="utf-8")
        return output

    def process_document(self, file_path: Path | str, metadata: dict | None = None) -> dict:
        text = self.read_document(file_path)
        if not text.strip():
            raise ValueError("El documento no contiene texto extraíble.")

        knowledge_object = self.build_knowledge_object(file_path, text, metadata)
